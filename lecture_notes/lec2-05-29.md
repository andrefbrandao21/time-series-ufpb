...
* **Metodologia "Pseudo Out-of-Sample Forecasting" (Previsão Fora da Amostra):**
    ...
    * O erro de previsão para $t+1$ é calculado como a diferença entre o valor observado $(Y_{t+1})$ e o valor previsto $(Ŷ_{t+1})$.
...
* **Erro Quadrático Médio (MSE - Mean Squared Error):**
    * Calculado como $$ \text{MSE} = \frac{1}{P} \sum_{t=1}^{P} (erro_t)^2 $$
...
* **R² Fora da Amostra (Out-of-Sample R² ou OOS R²):**
    * Fórmula: $$\text{OOS R}^2 = 1 - \frac{\sum (erro_{\text{modelo } i})^2}{\sum (erro_{\text{modelo benchmark}})^2}$$
    ...
    * Se o R² é próximo de zero, o modelo não prevê melhor que o modelo simples/benchmark.
    * Se o R² é maior que zero, a previsão do modelo é melhor que a do modelo simples.
...
* **Exercício de Riqueza Acumulada:**
    ...
    * Regra de decisão: Se a previsão do retorno em $t+1$ (feita em $t$) for positiva, compra-se o ativo. O resultado em $t+1$ será $1 * (1 + r).
